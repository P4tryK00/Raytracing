...
Color Renderer::renderPixel(int x, int y, const Scene& scene, const Camera& camera,
    ...
    // Wypuszczanie pakietu promieni (sub-pixel rays) zgodnie z wzorcem Samplera
    ...
    // Uśrednienie zakumulowanego światła fizycznego z przestrzeni liniowej
    Color finalColor = sum / static_cast<double>(samples.size());

    // --- TONE MAPPING (Mapowanie Tonów - Model Ekspozycji Fotograficznej) ---
    // Nieliniowa kompresja wysokich wartości natężenia światła (np. rozbłysków > 1.0) do przedziału [0.0, 1.0].
    // Zapobiega "przepaleniu" (clipping) obrazu i pozwala na fotorealistyczne wygasanie świateł.
    double exposure = 1.0;
    ...
    // Zwracany kolor wędruje do Image.cpp, gdzie zostanie na niego nałożona korekcja Gamma.
    return Color(r, g, b);
}

// Rekurencyjna funkcja śledzenia promienia (Whitted-style Ray Tracing).
// Bada kolizję promienia z obiektami w scenie i decyduje o przerwaniu lub kontynuacji optyki.
Color Renderer::trace(const Ray& ray, const Scene& scene, int depth) const {
    // 0.001 to 'Epsilon' z-buffera, a 1e9 to limit renderowania (Far Clip Plane).
    ...
    // Jeśli promień uciekł w pustkę, zwraca globalny kolor otoczenia (np. czarne niebo)
...
Color Renderer::shade(const IntersectionResult& hit, const Scene& scene, const Ray& ray, int depth) const {
    ...
    // Fizyczna separacja światła bazowego (dyfuzji) od rozbłysków lustrzanych.
    ...
    // Najprostsze przybliżenie zjawiska Global Illumination - rozproszonego światła odbijającego się od ścian.
    Color ambient = mat.baseColor * mat.Ka;
    baseColor += ambient;

    // Prekalkulowane znormalizowane wektory: Normalna (N) i Wektor Kamery/Widza (V)
    ...
    for (const auto& light : scene.getLights()) {
        ...
        // --- 2. CIENIE (Shadow Rays) ---
        // Dodanie małego przesunięcia (N * 0.001) zapobiega zjawisku samocieniowania (Shadow Acne),
        // wynikającemu z precyzji zmiennoprzecinkowej komputerów.
        ...
        // --- 4. SPECULAR (Rozbłysk Lustrzany - Model Phonga) ---
    ...
    if (depth > 0) {
        ...
        // --- DIELEKTRYKI (Szkło / Woda - Prawo Snella) ---
        if (mat.transparency > 0.0) {
            ...
            if (k >= 0.0) {
                ...
                // (Jeśli k < 0, mamy do czynienia z Całkowitym Wewnętrznym Odbiciem - TIR)
    ...
    // --- 5. ZASADA ZACHOWANIA ENERGII (Miksowanie końcowe) ---
    // Materiał nie może emitować więcej światła niż przyjął.
    ...
    finalColor = (baseColor * localWeight) +
                 specularColor + // Rozbłysk lustrzany dodajemy osobno (szkło nadal musi błyszczeć!)
                 (reflectedColor * mat.reflectivity) +
                 (refractedColor * mat.transparency);

    // Surowe ucięcie wartości nadmiarowych (zabezpieczenie warstwy niższej przed błędami formatowania)
